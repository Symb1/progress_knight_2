// I don't like disabling this (as it's a good rule) but... I don't have much of an option
...
function setCustomEffects() {
  ...
  brainwashing.getEffect = function() {
    let multiplier = 1 - getBaseLog(7, brainwashing.level + 1) / 10;
    if (multiplier < 0.1) multiplier = 0.1;
    return multiplier;
  };
  ...
  abyssManipulation.getEffect = function() {
    let multiplier = 1 - getBaseLog(7, abyssManipulation.level + 1) / 10;
    if (multiplier < 0.1) multiplier = 0.1;
    return multiplier;
  };
  ...
  galacticCommand.getEffect = function() {
    let multiplier = 1 - getBaseLog(7, galacticCommand.level + 1) / 10;
    if (multiplier < 0.1) multiplier = 0.1;
    return multiplier;
  };
  ...
  unholyRecall.getEffect = function() {
    const multiplier = unholyRecall.level * 0.00065;
    return multiplier;
  };
}
...
function getHappiness() {
  ...
  const happiness = meditationEffect() * butlerEffect() / mindseizeEffect() * multiverseFragment() * gameData.currentProperty.getEffect();
  return happiness;
}
...
function getEvilGain() {
  ...
  const evil = evilControl.getEffect() * bloodMeditation.getEffect() * absoluteWish.getEffect() * oblivionEmbodiment.getEffect() * yingYang.getEffect();
  return evil;
}
...
function getEssenceGain() {
  ...
  const essence = essenceControl.getEffect() * essenceCollector.getEffect();
  return essence;
...
function getGameSpeed() {
  ...
  const timeWarpingSpeed = gameData.timeWarpingEnabled ? timeWarping.getEffect() + temporalDimension.getEffect() * timeLoop.getEffect() : 1;
  const gameSpeed = baseGameSpeed * Number(!gameData.paused) * Number(isAlive()) * timeWarpingSpeed;
  return gameSpeed;
}
...
function createRow(templates, name, categoryInformation) {
  const row = templates.row.content.firstElementChild.cloneNode(true);
  const categoryType = categoryInformation.categoryType;
  const categoryName = categoryInformation.categoryName;
  row.getElementsByClassName("name")[0].textContent = name;
  row.getElementsByClassName("tooltipText")[0].textContent = tooltips[name];
  row.id = `row ${name}`;
  if (categoryType === itemCategories) {
    // I have no idea what is supposed to be happening here so I'm just leaving it
...
function createAllRows(categoryType, tableId) {
  ...
  for (const categoryName in categoryType) {
    ...
    category.forEach(name => {
      ...
      const row = createRow(templates, name, categoryInformation);
...
// Holy lord god almighty help this function
...
function updateTaskRows() {
  for (const key in gameData.taskData) {
    ...
    if (gameData.rebirthOneCount > 0) maxLevel.classList.remove("hidden");
    else maxLevel.classList.add("hidden");

    const progressFill = row.getElementsByClassName("progressFill")[0];
    progressFill.style.width = `${task.xp / task.getMaxXp() * 100}%`;

    if (task === gameData.currentJob || task === gameData.currentSkill) progressFill.classList.add("current");
    else progressFill.classList.remove("current");
...
function updateHeaderRows(categories) {
  for (const categoryName in categories) {
    ...
    if (gameData.rebirthOneCount > 0) maxLevelElement.classList.remove("hidden");
    else maxLevelElement.classList.add("hidden");
...
function getCategoryFromEntityName(categoryType, entityName) {
  ...
  throw new Error(`${entityName} is not in any category at function getCategoryFromEntityName`);
}
...
function checkSkillSkipped(skill) {
  ...
  const isSkillSkipped = row.getElementsByClassName("checkbox")[0].checked;
...
function setSkillWithLowestMaxXp() {
  ...
  enabledSkills.sort((lhs, rhs) => lhs.getMaxXp() / lhs.getXpGain() - rhs.getMaxXp() / rhs.getXpGain());

  const skillName = enabledSkills[0].name;
...
function getKeyOfLowestValueFromDict(dict) {
  ...
  throw new Error("No key found in function getKeyOfLowestValueFromDict");
}
...
function formatCoins(coins, element) {
  ...
  for (const tier of tiers) {
    ...
    const text = (coins > 1e9 && i > 0) ? "" : `${format(String(x), 2) + tier} `;
...
function removeSpaces(string) {
  const changedString = string.replace(/ /ug, "");
...
function getLifespan() {
  ...
  const lifespan = baseLifespan * immortality.getEffect() * superImmortality.getEffect() * abyss.getEffect() * cosmicLongevity.getEffect() * higherDimensions.getEffect();
  return lifespan;
}
